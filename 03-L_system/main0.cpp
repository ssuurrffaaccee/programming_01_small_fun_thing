...
int main() {
  try {
    ...
    LSystem system;
    system.init({"F", "-", "F", "-", "F", "-", "F"}, 2);
    system.add_rule("F", {"F", "-", "F", "+", "F", "+", "F", "F", "-", "F", "-",
                          "F", "+", "F"});
    LSystemViewer system_viewer{system};
    uint32_t dump_size{1000};
    float one_step_length = 30.0f / dump_size;
    Turtle turtle = defaul_turtle(make_vec(0.25f,0.25f),one_step_length);
    float degree = 90;
